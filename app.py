...
with st.expander("⚙️ Parámetros de tolerancia"):
    ...
    multiplo_redondo = st.selectbox(
        "Múltiplo para valor 'redondo' (alta ambigüedad)", [50000, 100000], index=1
    )
...
if archivo_subido is not None:
    try:
        with st.spinner("Ejecutando motor de reglas CLM..."):
            ...
            col_C = 'Clase de documento' if 'Clase de documento' in df.columns else (
                'Clase doc.' if 'Clase doc.' in df.columns else None
            )
            col_D = 'Fe.contabilización' if 'Fe.contabilización' in df.columns else (
                'Fecha de documento' if 'Fecha de documento' in df.columns else None
            )
            ...
            col_novedad = 'novedad' if 'novedad' in df.columns else (
                'Novedad' if 'Novedad' in df.columns else None
            )
            ...
            for _, row in df.iterrows():
                ...
                if "cuenta de mayor" in asig_val.lower():
                    ...
                    if m:
                        current_bank = mapeo_cuentas_banco.get(
                            m.group(1), f"CUENTA {m.group(1)} (sin mapear)"
                        )
            ...
            def obtener_ref_homologada(row):
                ...
                for n in n8:
                    if n in mapeo_datafono_ref:
                        return mapeo_datafono_ref[n]
                n4 = re.findall(r'\b\d{4}\b', texto)
                for n in n4:
                    if n in mapeo_datafono_ref.values():
                        return n
                return None
            ...
            def gate_seguridad(id40, id50, exigir_importe_exacto=True, tolerancia_valor=None):
                ...
                if dif_dias > TOPE_DIAS_ALERTA:
                    resultado['motivo'] = (
                        f"Diferencia de fecha F fuera de rango ({dif_dias} días > {TOPE_DIAS_ALERTA})"
                    )
                    return resultado
            ...
            def clasificar_y_registrar(id40, id50, base_txt):
                res = gate_seguridad(
                    id40, id50,
                    exigir_importe_exacto=True,
                    tolerancia_valor=tol_valor_purpura,
                )
                if not res['ok']:
            ...
            if usar_ipcb:
                ...
                df_cb = df[
                    (df[col_C].astype(str).str.upper() == 'CB') &
                    (df[col_G] == '50') &
                    df['Ref_H_Homologada'].notna()
                ]
                ...
                if not df_ip.empty and not df_cb.empty:
                    ...
                    con_tol = m[(m['DifV'] > 0) & (
                        (m['DifV'] <= tol_valor_abs_general) |
                        (m['Pct'] <= tol_valor_pct_general)
                    )]
                    ...
                    def procesar_grupo_ip(fila, es_exacto):
                        ...
                        if es_exacto:
                            ...
                            txt = (
                                f"Cruce múltiple homologado ({len(ip_ids)} IP = {len(cb_ids)} CB). "
                                f"Ref. homologada: {rh}."
                            )
                        else:
                            ...
                            txt = (
                                f"Sugerencia IP/CB con diferencia de valor (${fila['DifV']:,.0f} / "
                                f"{fila['Pct']*100:.2f}%). Suma {len(ip_ids)} IP vs {len(cb_ids)} CB. "
                                f"Ref. homologada: {rh}."
                            )
                        ...
                        if len(match_importe) == 1:
                            ...
                            for idx in (fila_ip['ID_Linea'], id_cb):
                                ...
                                df.loc[df['ID_Linea'] == idx, 'Comentario'] = (
                                    f"IP conciliado por misma Zona ({sector_z}) e importe exacto."
                                )
                            usados.update([fila_ip['ID_Linea'], id_cb])
                        elif len(match_importe) > 1:
                            escribir_estado(
                                [fila_ip['ID_Linea']],
                                'Sugerencia - IP por Zona (varios candidatos)',
                                forzar=False,
                            )
                            escribir_candidatos(
                                fila_ip['ID_Linea'],
                                f"Zona {sector_z}: candidatos CB con mismo importe: {docs_candidatos}",
                            )
                            escribir_comentario(
                                fila_ip['ID_Linea'],
                                f"IP con misma Zona ({sector_z}) pero varios CB candidatos con el mismo importe exacto: requiere revisión manual.",
                                append=False,
                            )
            ...
            for idl in ip_sin_resolver['ID_Linea']:
                escribir_comentario(
                    idl,
                    "PDV (IP): requiere referencia homologada de base de datos o coincidencia por Zona (Regla 9) para conciliar. Sin coincidencia por ahora.",
                    append=False,
                )
            ...
            df_nequi_dz = df[
                (df[col_G] == '40') & (df['Es_Nequi'] == True) & (~df['ID_Linea'].isin(usados))
            ]
            df_cb_disponible = df[
                (df[col_G] == '50') & (~df['ID_Linea'].isin(usados))
            ]
            if usar_ipcb:
                df_cb_disponible = df_cb_disponible[
                    df_cb_disponible[col_C].astype(str).str.upper() != 'IP'
                ]
            ...
            if not df_nequi_dz.empty and not df_cb_disponible.empty:
                for (banco_g, fecha_g), grupo_dz in df_nequi_dz.groupby([col_banco, 'Fecha_F']):
                    ...
                    if n_dz == n_cb and total_dz == total_cb:
                        texto_grupo = (
                            f"total DZ=${total_dz:,.0f}; total CB=${total_cb:,.0f}; "
                            f"cruce FIFO por B ({n_dz} lineas)."
                        )
                        for i in range(n_parejas):
                            ...
                            for idx in (id40, id50):
                                df.loc[df['ID_Linea'] == idx, 'Estado_Conciliacion'] = (
                                    'Conciliado - Regla 8 Nequi (total y FIFO)'
                                )
                                df.loc[df['ID_Linea'] == idx, 'Candidatos_Conciliacion'] = texto_cand
                                df.loc[df['ID_Linea'] == idx, 'Comentario'] = (
                                    f"Regla 8 Nequi: {texto_grupo}"
                                )
                        ...
                        motivo = (
                            f"cantidad DZ={n_dz} vs CB={n_cb}" if n_dz != n_cb else (
                                f"totales distintos (DZ=${total_dz:,.0f} vs CB=${total_cb:,.0f}, dif=${diff_total:,.0f})"
                            )
                        )
                        docs_dz = resumen_docs(dz_ord)
                        docs_cb = resumen_docs(cb_ord)
                        texto_cand = (
                            f"Grupo NEQUI {banco_g} {fecha_g}: DZ candidatos: {docs_dz} || CB candidatos: {docs_cb}"
                        )
                        comentario = (
                            f"Regla 8 Nequi: grupo NO cuadra exacto ({motivo}). "
                            "No se concilia automático; requiere revisión manual del grupo completo."
                        )
                        ...
                        for _, fila in pd.concat([dz_ord, cb_ord]).iterrows():
                            ...
                            escribir_estado(
                                [idl],
                                'Sugerencia - Regla 8 Nequi (revisar total de grupo)',
                                forzar=False,
                            )
            ...
            # =====================================================
            # 6. REGLA 6 EXPLÍCITA — RECLASIFICACIÓN DE BANCO
            # =====================================================
            ...
            def emparejar_reclasificacion(sub40, sub50, llave40, llave50, base_txt):
                ...
                merged = pd.merge(
                    s40, s50,
                    left_on=llave40 + ['_pos'],
                    right_on=llave50 + ['_pos'],
                    suffixes=('_40', '_50'),
                )
            ...
            emparejar_1a1_por_llave(
                df_40, df_50,
                [col_banco, 'Abs_I', col_A],
                [col_banco, 'Abs_I', col_H],
            ...
            emparejar_1a1_por_llave(
                df_40[df_40['A_Limpia'] != ''],
                df_50[df_50['H_Limpia'] != ''],
                [col_banco, 'Abs_I', 'A_Limpia'],
                [col_banco, 'Abs_I', 'H_Limpia'],
                "Regla 1 (limpia): Asignación limpia coincide con Referencia limpia (ej. E3110→3110).",
            )

            # =====================================================
            # 8. REGLA 2 — SECTORIZACIÓN
            # =====================================================
            df_40 = df_40[~df_40['ID_Linea'].isin(usados)]
            df_50 = df_50[~df_50['ID_Linea'].isin(usados)]
            ...
            if not d40_sect.empty and not d50_sect.empty:
                for grp, sub40 in d40_sect.groupby([col_banco, 'Abs_I', 'Sector']):
                    b, imp, sector = grp
                    sub50 = d50_sect[
                        (d50_sect[col_banco] == b) &
                        (d50_sect['Abs_I'] == imp) &
                        (d50_sect['Sector'] == sector)
                    ]
                    if sub50.empty:
                        continue
                    ...
                    if not rem40.empty or not rem50.empty:
                        ...
                        for _, r in rem40.iterrows():
                            df.loc[df['ID_Linea'] == r['ID_Linea'], 'Estado_Conciliacion'] = (
                                f"Sugerencia - Sectorización desbalanceada ({sector})"
                            )
                            df.loc[df['ID_Linea'] == r['ID_Linea'], 'Candidatos_Conciliacion'] = (
                                f"{formato_linea(r['ID_Linea'])} | Candidatos posibles: {docs50_txt}"
                            )
                            df.loc[df['ID_Linea'] == r['ID_Linea'], 'Comentario'] = (
                                f"Sector '{sector}' desbalanceado ({len(rem40)} vs {len(rem50)}). "
                                f"Créditos candidatos: {docs50_txt}"
                            )
                        for _, r in rem50.iterrows():
                            ...
                            df.loc[df['ID_Linea'] == r['ID_Linea'], 'Candidatos_Conciliacion'] = (
                                f"{formato_linea(r['ID_Linea'])} | Candidatos posibles: {docs40_txt}"
                            )
                            df.loc[df['ID_Linea'] == r['ID_Linea'], 'Comentario'] = (
                                f"Sector '{sector}' desbalanceado ({len(rem50)} vs {len(rem40)}). "
                                f"Débitos candidatos: {docs40_txt}"
                            )

            # =====================================================
            # 9. EXCEPCIÓN NEQUI (individual)
            # =====================================================
            df_40 = df_40[~df_40['ID_Linea'].isin(usados)]
            df_50 = df_50[~df_50['ID_Linea'].isin(usados)]
            ...
            for _, r40 in df_nequi_40.iterrows():
                ...
                if r40['Sector'] != 'Sin clasificar':
                    candidatos_filtrados = candidatos_50[
                        (candidatos_50['Sector'] == r40['Sector']) |
                        (candidatos_50['Sector'] == 'Sin clasificar')
                    ]
                ...
                if len(exactos) == 1:
                    ...
                    texto_cand = f"{formato_linea(id40)} | {formato_linea(id50)}"
                    df.loc[df['ID_Linea'] == id40, 'Estado_Conciliacion'] = (
                        'Conciliado - Excepción Nequi (candidato único)'
                    )
                    df.loc[df['ID_Linea'] == id50, 'Estado_Conciliacion'] = (
                        'Conciliado - Excepción Nequi (candidato único)'
                    )
                    for idx in (id40, id50):
                        df.loc[df['ID_Linea'] == idx, 'Candidatos_Conciliacion'] = texto_cand
                ...
                con_tol = candidatos_50[
                    candidatos_50['_dif'] <= tol_valor_purpura
                ].sort_values('_dif')
                if len(con_tol) == 1:
                    ...
                    for idx in (id40, id50):
                        df.loc[df['ID_Linea'] == idx, 'Candidatos_Conciliacion'] = texto_cand
                        df.loc[df['ID_Linea'] == idx, 'Comentario'] = (
                            f"Excepción Nequi con diferencia de valor: ${dif_v:,.0f} (tope morado ${tol_valor_purpura:.0f}). "
                            "Candidato único en banco+fecha."
                        )
                    usados.update([id40, id50])
                    continue
                ...
                if len(exactos) > 1 or len(con_tol) > 1:
                    ...
                    df.loc[df['ID_Linea'] == id40, 'Estado_Conciliacion'] = (
                        'Sugerencia - Excepción Nequi ambigua'
                    )
                    df.loc[df['ID_Linea'] == id40, 'Candidatos_Conciliacion'] = (
                        f"{formato_linea(id40)} | Candidatos posibles: {docs_txt}"
                    )
            ...
            # =====================================================
            # 10. REGLA 4 — DZ POSICIONES MÚLTIPLES
            # =====================================================
            df_40 = df_40[~df_40['ID_Linea'].isin(usados)]
            df_50 = df_50[~df_50['ID_Linea'].isin(usados)]
            ...
            for b_doc, grupo in dz_repetidos.groupby(col_B):
                for _, linea in grupo.iterrows():
                    ...
                    candidatos['_dif_dias'] = candidatos['Fecha_F'].apply(
                        lambda f: abs((f - linea['Fecha_F']).days)
                        if pd.notna(f) and pd.notna(linea['Fecha_F'])
                        else 999
                    )
                    ...
                    candidatos['_mismo_sector'] = (
                        (candidatos['Sector'] == linea['Sector']) &
                        (linea['Sector'] != 'Sin clasificar')
                    )
                    candidatos['_mismo_A_H'] = (
                        candidatos[col_H].astype(str).str.strip() == str(linea[col_A]).strip()
                    )
                    ...
                    texto_cand_final = (
                        f"Línea 40: Doc={int(linea[col_B])}, I=${linea['Abs_I']:,.0f}, F={linea[col_F]} || Candidatos: "
                        + " ; ".join(lineas_candidatos)
                    )
                    df.loc[df['ID_Linea'] == idl, 'Estado_Conciliacion'] = (
                        'Sugerencia - DZ posiciones múltiples (verificar)'
                    )
            ...
            # =====================================================
            # 11. ÚLTIMO RECURSO: FIFO CONTROLADO
            # =====================================================
            df_40 = df_40[~df_40['ID_Linea'].isin(usados)]
            df_50 = df_50[~df_50['ID_Linea'].isin(usados)]
            ...
            for grp, sub40 in pendientes_40.groupby([col_banco, 'Abs_I', col_F, 'Sector']):
                b, imp, f, sector = grp
                sub50 = pendientes_50[
                    (pendientes_50[col_banco] == b) &
                    (pendientes_50['Abs_I'] == imp) &
                    (pendientes_50[col_F] == f) &
                    (pendientes_50['Sector'] == sector)
                ]
                if sub50.empty:
                    continue
                ...
                for i in range(n_pares):
                    ...
                    for idx in (id40, id50):
                        df.loc[df['ID_Linea'] == idx, 'Estado_Conciliacion'] = (
                            'Conciliado - FIFO controlado (última instancia)'
                        )
                        df.loc[df['ID_Linea'] == idx, 'Candidatos_Conciliacion'] = texto_cand
                        df.loc[df['ID_Linea'] == idx, 'Comentario'] = (
                            'Emparejado por FIFO controlado: misma fecha F, mismo sector, banco e importe.'
                        )
                ...
                for _, fila in sobrantes40.iterrows():
                    ...
                    if bool(fila['B_Repite']):
                        df.loc[df['ID_Linea'] == idl, 'Estado_Conciliacion'] = (
                            'Sugerencia - DZ multiposición sin cruce (verificar)'
                        )
                        df.loc[df['ID_Linea'] == idl, 'Comentario'] = (
            ...
            # =====================================================
            # 12. CIERRE — PENDIENTES
            # =====================================================
            sin_p = df['Estado_Conciliacion'] == 'Pendiente'
            if usar_ipcb:
                es_ip = df[col_C].astype(str).str.upper() == 'IP'
                df.loc[sin_p & es_ip & (df['Comentario'] == ''), 'Comentario'] = (
                    'Sin coincidencia - PDV (requiere referencia homologada)'
                )
                df.loc[sin_p & ~es_ip & (df['Comentario'] == ''), 'Comentario'] = (
                    'Sin coincidencia ni sugerencia que cumpla reglas de seguridad - revisión manual completa'
                )
            else:
                df.loc[sin_p & (df['Comentario'] == ''), 'Comentario'] = (
                    'Sin coincidencia ni sugerencia que cumpla reglas de seguridad - revisión manual completa'
                )
            ...
            output = io.BytesIO()
            b_unicos = [
                b for b in df_final[col_banco].unique()
                if str(b).strip().lower() not in ('', 'nan')
            ]
            orden_cuentas = [
            ...
            nombres_ordenados = [
                mapeo_cuentas_banco.get(c, f"CUENTA {c} (sin mapear)") for c in orden_cuentas
            ]

            def orden_banco(bs):
                bs = str(bs).strip()
                if bs in nombres_ordenados:
                    return nombres_ordenados.index(bs)
            ...
            def hoja_segura(writer, df_hoja, nombre, estilo=True):
                ...
                try:
                    if estilo and not df_hoja.empty:
                        df_hoja.style.apply(
                            lambda row: color_fila(df_hoja.loc[row.name]), axis=1
                        ).to_excel(writer, index=False, sheet_name=nf)
                    else:
                    ...
                    try:
                        df_hoja.to_excel(writer, index=False, sheet_name=nf)
                        advertencias.append(
                            f"Hoja '{nf}': sin colores por error de formato ({e1})."
                        )
                    except Exception as e2:
                        advertencias.append(
                            f"Hoja '{nf}': no se pudo escribir ({e2})."
                        )

            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                total_filas = len(df_final)
                total_azul = int(
                    df_final['Estado_Conciliacion'].str.contains('Conciliado', na=False).sum()
                )
                total_verde = int(
                    df_final['Estado_Conciliacion'].str.contains(
                        'DZ multiposición|DZ posiciones múltiples',
                        na=False,
                        regex=True,
                    ).sum()
                )
                total_salmon = int(
                    df_final['Estado_Conciliacion'].str.contains(
                        'Diferencia de fecha', na=False
                    ).sum()
                )
                total_morado = int(
                    df_final['Estado_Conciliacion'].str.contains(
                        'Diferencia de valor', na=False
                    ).sum()
                )
                total_durazno = int(
                    df_final['Estado_Conciliacion'].str.contains(
                        'Reclasificación', na=False
                    ).sum()
                )
                ...
                resumen = pd.DataFrame({
                    ...
                    "Valor": [
                        ...
                        total_azul, total_verde, total_salmon, total_morado,
                        total_durazno, total_pendiente,
                        ...
                        filas_excluidas,
                        int(df_final['B_Repite'].sum())
                        if 'B_Repite' in df_final.columns else 0,
                    ],
                })
                ...
                estados_alerta = [
                    'Diferencia de fecha', 'Diferencia de valor', 'Reclasificación de banco',
                ]
                mask_alerta = df_nov['Estado_Conciliacion'].isin(estados_alerta)
                mask_sin_candidato = df_nov['Candidatos_Conciliacion'].astype(str).str.strip().isin([
                    '', 'nan', 'None',
                ])
                ...
                if not df_nov.empty:
                    ...
                    hoja_segura(
                        writer,
                        pd.DataFrame(columns=columnas_visibles),
                        'NOVEDADES_Y_PENDIENTES_40',
                        estilo=False,
                    )
            ...
            st.success("¡Conciliación completada con el motor de reglas CLM v25 (corregido)!\n")
            if not cuadre_ok:
            ...
            st.download_button(
                ...
                file_name="Conciliacion_CLM_v25_corregida.xlsx",
